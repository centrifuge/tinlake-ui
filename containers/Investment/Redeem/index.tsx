...
class InvestorRedeem extends React.Component<Props, State> {
  ...
  render() {
    ...
    return <Box basis={'1/4'} gap="medium" margin={{ right: 'large' }}>
      <Box gap="medium">
        {tranche.type === 'senior' && <Erc20Widget input={true} fieldLabel="Redeem token" limit={BN.min(maxRedeemAmount,tokenBalance).toString()} tokenData={dropToken} precision={18} onValueChanged={(value : string) =>
                this.setState({ redeemAmount: displayToBase(value, 18) })}
                errorMessage="Max redeem amount exceeded" />}
        {tranche.type === 'junior' && <Erc20Widget input={true} fieldLabel="Redeem token" limit={BN.min(maxRedeemAmount,tokenBalance).toString()} tokenData={tinToken} precision={18} onValueChanged={(value : string) =>
                this.setState({ redeemAmount: displayToBase(value, 18) })}
                errorMessage="Max redeem amount exceeded" />}
      </Box>
      <Box align="start">
